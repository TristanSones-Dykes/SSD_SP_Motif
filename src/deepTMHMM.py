...
def extract_deepTMHMM(origin: str, max_regions: int = None) -> pd.DataFrame:
    ...
    proteins = [[re.sub('\s+', ' ', line).strip() for line in protein.split("\n")[1:-1]] for protein in proteins]
    ...
    region_count = [(protein.count("TMhelix"), protein.count("signal")) for protein in col_2]
    ...
    window_df = pd.DataFrame(columns = ["seqID", "start", "end", "window_type", "protein_type"])
    window_df = window_df.astype({"seqID": str, "start": int, "end": int, "window_type": str, "protein_type" : str}) # setting datatypes

    # grab protein data with tm, sig, and both regions 
    tm_proteins, sig_proteins, tm_sig_proteins = [protein_dat[i] for i in tm if i not in tm_sig], [protein_dat[i] for i in sig if i not in tm_sig], [protein_dat[i] for i in tm_sig]
    ...
    for protein in sig_proteins:
        ...
        # append to dataframe
        new_row = pd.DataFrame([[seqID, start, end, window_type, protein_type]], columns = ["seqID", "start", "end", "window_type", "protein_type"])
